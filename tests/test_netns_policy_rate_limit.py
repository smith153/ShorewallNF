"""Netns behavioral proof of the policy LIMIT:BURST column (#408, epic #400, ADR-0006/0007).

Drives that an nft ``limit rate`` emitted on a *policy* fall-through actually throttles the
traffic it defaults: *new connections* under the configured rate take the policy action (ACCEPT
here), new connections over it fall through past the limiter to the fail-closed ``forward`` base
policy (DROP). The limiter rides the policy rule itself, not a feature rule.

Why distinct flows, not one long ping: a policy rate-limits *new connections* it matches, and the
policy fall-through sits after the base established/related accept (ADR-0005). So only the
**first** packet of a conntrack flow is NEW and ever reaches the limiter; a single ``ping -c N`` is
one ICMP flow (one id), whose packets 2..N are ESTABLISHED and admitted by the base-accept *ahead*
of the policy — they never touch it. To exercise the limiter we therefore fire many **separate**
``ping -c1`` processes: each has its own ICMP echo id, hence a distinct conntrack flow whose sole
packet is NEW and must clear the limiter to get through.

Topology: a router namespace between a client and a server, each on its own RFC 5737 subnet. The
client pings the server through the router; the router's ``forward`` chain has a cli→srv ACCEPT
policy carrying the rate, and the base ``forward`` policy is DROP.

Signals (same policy, opposite rates — the contrast is the proof, not either half alone):
  * a slow, under-rate sequence of distinct new flows gets every probe through — the policy admits
...
_CLIENT = nh.Endpoint(
    name="snf408_c", iface="v_cli", peer="p_cli", addr4="192.0.2.2/24", router4="192.0.2.1/24"
)
_SERVER = nh.Endpoint(
    name="snf408_s", iface="v_srv", peer="p_srv", addr4="198.51.100.2/24", router4="198.51.100.1/24"
)
_TOPO = nh.Topology(router="snf408_r", endpoints=(_CLIENT, _SERVER))
...
# A cli->srv ACCEPT policy at a low rate; every new connection over the limit falls through past
# the limiter to the base forward DROP policy. A small burst keeps the under-rate case
# deterministic while the over-rate burst still swamps it.
_RATE = RateLimit(rate=2, interval="second", burst=2)
_POLICIES = (Policy(source="cli", dest="srv", action="ACCEPT", rate=_RATE),)
...
@pytest.mark.netns
@_requires_netns
def test_policy_rate_limit_throttles_above_rate_and_passes_below() -> None:
    rs = Ruleset(zones=_ZONES, policies=_POLICIES)
    with nh.NetnsSandbox(_TOPO) as sb:
        sb.load(rs)
        # Under-rate: three distinct new flows a second apart are all admitted (the 2/second bucket
        # keeps up), proving the policy passes below-limit new connections.
