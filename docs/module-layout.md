...
| Applier | `shorewallnf/applier.py` | shell (runs `nft --check`, atomically loads, saves/restores the persisted ruleset, [ADR-0010](adr/0010-atomic-scoped-replace.md)/[ADR-0030](adr/0030-reboot-persistence-model.md); post-load kernel sysctls for `IP_FORWARDING`/`LOG_MARTIANS`/`ROUTE_FILTER`, fail-closed with rollback, [ADR-0062](adr/0062-applier-kernel-sysctl-mutation.md); read-only `list_ruleset` query for the `show` verbs, [ADR-0065](adr/0065-operational-visibility-output-format.md)) | **present** |
| Renderer | `shorewallnf/renderer.py` | core (pure: nft-JSON → readable text) | **present** (nft `list` JSON → the Option B annotated columnar `show` output, [ADR-0065](adr/0065-operational-visibility-output-format.md)) |

Cross-cutting:

| Concern | Module | Status |
|---------|--------|--------|
| CLI entrypoint | `shorewallnf/cli.py` | **present** (`check`/`compile`/`apply`/`start`/`reload`/`restart`/`stop`/`clear`/`restore` verbs; read-only `show`/`list`/`ls` group with `show rules`, [ADR-0065](adr/0065-operational-visibility-output-format.md)) |
